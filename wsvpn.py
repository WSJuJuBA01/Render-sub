...
# Защита от ошибки Telegram API 400 "can't parse entities ...":
# если в тексте встречаются спецсимволы Markdown (_, *, `, [ и т.п.) из
# пользовательских данных (имена, детали логов и т.д.), запрос с
# parse_mode упадёт. Ниже — обёртки, которые в этом случае автоматически
# повторяют отправку тем же текстом, но уже без parse_mode (plain text),
# чтобы сообщение в любом случае дошло до пользователя.

def escape_markdown(text):
    """Экранирует спецсимволы legacy Markdown в произвольном тексте,
    который подставляется в форматированное сообщение (например, имя
    пользователя или произвольная строка)."""
    if text is None:
        return ""
    text = str(text)
    for ch in ('_', '*', '`', '['):
...
def return_db_connection(conn):
    if not conn:
        return
    try:
        # Если соединение осталось в "aborted"/незакоммиченном состоянии
        # после ошибки (например, конфликт уникального токена), без
        # rollback оно вернётся в пул "отравленным" и следующий, кто его
        # возьмёт, получит ошибку "current transaction is aborted" на
        # ровном месте. Поэтому всегда подчищаем перед возвратом.
        if conn.closed == 0 and conn.status != psycopg2.extensions.STATUS_READY:
            conn.rollback()
    except Exception as e:
        print(f"[return_db_connection] Ошибка rollback: {e}")
        try:
            conn.close()
        except:
            pass
        ...
        for _ in range(5):
            ...
            try:
                ...
                if result:
                    token = result[0]
                    break
                # token уже был выставлен параллельно другим запросом
                ...
                if 'unique' in str(e).lower():
                    # коллизия токена — пробуем сгенерировать новый
                    continue
                raise
        if not token:
            return None
